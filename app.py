...
def create_app():
    ...
    def signup():
        ...
        lock_result = db.execute("SELECT changes() AS changed").fetchone()
        ...
        try:
            status = get_status_for_slot(slot_id, week_date, is_member=is_member)
    ...
    def admin_dashboard():
        ...
        return render_template(
            ...
            themes=THEMES,
            current_theme_key=get_theme_key(),
            bg_styles=BG_STYLES,
        )
    ...
    def admin_edit_signup(signup_id):
        db = get_db()
        signup = db.execute(
            "SELECT id, name, slot_id, status, is_member FROM signups WHERE id = ?",
            (signup_id,),
        ).fetchone()
        if not signup:
        ...
        if request.method == "POST":
            ...
            if not name or not slot_id.isdigit():
                flash("Bitte Name und Slot angeben.", "error")
                return redirect(url_for("admin_edit_signup", signup_id=signup_id))

            db.execute(
                """
                UPDATE signups
                SET name = ?, name_normalized = ?, slot_id = ?, status = ?, is_member = ?
                WHERE id = ?
                """,
                (name, normalize_name(name), int(slot_id), status, is_member, signup_id),
            )
            db.commit()
            flash("Anmeldung aktualisiert.", "success")
            return redirect(url_for("admin_dashboard"))

        slots = db.execute("SELECT id, label FROM slots ORDER BY sort_order").fetchall()
        return render_template("admin_edit_signup.html", signup=signup, slots=slots)
    ...
    @admin_required
    def admin_qrcode():
        return render_template("admin_qrcode.html")
    ...
    def admin_events():
        db = get_db()
        events = db.execute(
            "SELECT DISTINCT week_date FROM signups ORDER BY week_date DESC"
        ).fetchall()
    ...
    def admin_system():
        ...
        return render_template("admin_system.html", settings=settings)
...
def get_status_for_slot(slot_id, week_date, is_member=1):
    db = get_db()
    mode = get_waitlist_mode()
    ...
    if confirmed < slot["max_players"]:
        if mode == "guests_only" and is_member == 0:
            return "waitlist"
        return "confirmed"

    if mode == "open_for_all":
        return "confirmed"
    ...
    limit = get_waitlist_limit()
    if limit <= 0 and mode != "guests_only":
        return "waitlist_full"
    if waitlist < limit:
        return "waitlist"
    return "waitlist_full"
...
def init_db():
    ...
    defaults = {
        ...
        "intro_text": DEFAULT_INTRO_TEXT,
        "waitlist_limit": str(DEFAULT_WAITLIST_LIMIT),
        "slot_selection": DEFAULT_SLOT_SELECTION,
        "waitlist_mode": DEFAULT_WAITLIST_MODE,
    }
